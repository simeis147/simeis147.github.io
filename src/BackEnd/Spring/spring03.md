...
* 作用:在不惊动原始设计的基础上为其进行功能增强。这句话就是前面咱们标红的那句话。
...
代码的内容相信大家都能够读懂，我们就不过多的解释了，对于`save`方法中有计算万次执行消耗的时间。
...
案例设定：测定接口执行效率，但是这个案例稍微复杂了点，我们对其进行简化。

简化设定：在接口执行前输出当前系统时间。

对于SpringAOP的开发有两种方式，XML 和 **注解**，我们使用哪个呢?

应用现在注解使用的比较多，所以本次课程就采用注解完成AOP的开发。

对上面的内容一句话概括就是:使用SpringAOP的注解方式完成在方法执行的前打印出当前系统时间。
...
* 接下来，我们要使用SpringAOP的方式在不改变update方法的前提下让其具有打印系统时间的功能。
...
![1630146885493](./assets/spring03/1630146885493.png)

* 因为`spring-context`中已经导入了`spring-aop`,这用到的是Maven的依赖传递,所以不需要再导入`spring-aop`
* 导入AspectJ的jar包,AspectJ是AOP思想的一个具体实现，Spring有自己的AOP实现，但是相比于AspectJ来说比较麻烦些，所以我们直接采用Spring整合ApsectJ的方式进行AOP开发。
...
AOP的入门案例已经完成，对于刚才案例的执行过程，我们就得来分析分析，这一节我们主要讲解两个知识点:`AOP工作流程`和`AOP核心概念`。这节核心概念是对前面核心概念的补充。
...
为了验证IOC容器中创建的对象和我们刚才所说的结论是否一致，首先来把结论理出来:
...
不能直接打印对象，从上面两次结果中可以看出，直接打印对象走的是对象的toString方法，不管是代理对象还是不是代理对象打印的结果都是一样的，原因是内部都toString方法进行了重写。
...
![ ](./assets/spring03/1630155937718.png)
...
![ ](./assets/spring03/1630166147697.png)
...
![ ](./assets/spring03/1630168248052.png)
