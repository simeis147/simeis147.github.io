...
整个环境的内容暂时可以不用关注，最主要的类为:`BookDaoImpl`
...
当在App类中从容器中获取bookDao对象后，分别执行其`save`,`delete`,`update`和`select`方法

![ ](./assets/spring03/1630143927489.png)

::: info 疑问?

* 对于计算万次执行消耗的时间只有save方法有，为什么delete和update方法也会有呢?

* delete和update方法有，那什么select方法又没有呢?

:::

> 这个案例中其实就使用了Spring的AOP，在不惊动(改动)原有设计(代码)的前提下，想给谁添加功能就给谁添加

#### 无入侵式 / 无侵入式

![ ](./assets/spring03/1630144353462.png)

1. BookServiceImpl中有`save`,`update`,`delete`和`select`方法，这些方法称为**连接点**

1. `update`和`delete`只有打印，但是在运行的时候有该功能，`update`和`delete`方法都已经被增强，所以对于需要增强的方法叫**切入点**

1. 执行BookServiceImpl的`update`和`delete`方法的时候都被添加了一个计算万次执行消耗时间的功能，将这个功能抽取到一个方法中，换句话说就是存放共性功能的方法，叫**通知**

1. 通知是要增强的内容，会有多个，切入点是需要被增强的方法，也会有多个，那哪个切入点需要添加哪个通知，就需要提前将它们之间的关系描述清楚，那么对于通知和切入点之间的关系描述，叫**切面**

1. 通知是一个方法，方法不能独立存在需要被写在一个类中，这个类叫**通知类**
...
![ ](./assets/spring03/1630167092142.png)

**说明:**

目前打印save方法的时候，因为方法中有打印系统时间，所以运行的时候是可以看到系统时间  
但是对于update方法来说，就没有该打印系统时间的功能  
接下来，使用SpringAOP的方式在不改变update方法的前提下让其具有打印系统时间的功能  
...
![ ](./assets/spring03/1630146885493.png)

* 因为`spring-context`中已经导入了`spring-aop`，这用到的是Maven的依赖传递，所以不需要再导入`spring-aop`
* 导入AspectJ的jar包，AspectJ是AOP思想的一个具体实现，Spring有自己的AOP实现，但是相比于AspectJ来说比较麻烦些
...
![ ](./assets/spring03/1630148447689.png)
...
![ ](./assets/spring03/1630147945888.png)
...
![ ](./assets/spring03/1630151682428.png)

* 上面有两个切入点的配置，但是第一个`ptx()`并没有被使用，所以不会被读取
...
* 要被实例化bean对象的类中的方法和切入点进行匹配
  ...
  ![ ](./assets/spring03/1630152538083.png)
  ...
  * 匹配成功，创建原始对象（**目标对象**）的**代理**对象，如:`BookDao`
    ...
    * 因为要对目标对象进行功能增强，而采用的技术是动态代理，所以会为目标对象创建一个代理对象
...
> 1. 要执行的方法，不被定义的切入点包含，即对该方法不进行增强，打印当前类的getClass()方法
>
> 2. 要执行的方法，被定义的切入点包含，即对该方法进行增强，打印出当前类的getClass()方法
>
> 3. 观察两次打印的结果
...
![ ](./assets/spring03/1630154495165.png)

##### 步骤4:修改MyAdvice类，增强

因为定义的切入点中，被修改成`update`,所以BookDao中的update方法在执行的时候，就会被增强  
所以容器中的对象应该是是目标对象的代理对象
...
![ ](./assets/spring03/1630154625564.png)
...
![ ](./assets/spring03/1630155937718.png =600x)

对于AOP中切入点表达式，分别是`语法格式`、`通配符`和`书写技巧`
...
![ ](./assets/spring03/1630156172790.png)
...
* `+` ：专用于匹配子类类型
  ...
  使用率较低，描述子类的，表示所有以Service结尾的接口的子类

---

![ ](./assets/spring03/1630163744963.png)
...
![ ](./assets/spring03/1630164718080.png)
...
* 前置通知
* 后置通知
* **环绕通知** <Badge text="重点" color="#242378" /> 
* 返回后通知 <Badge text="了解" color="#777" /> 
* 抛出异常后通知 <Badge text="了解" color="#777" /> 

:::

![ ](./assets/spring03/1630166147697.png =600x)
...
![ ](./assets/spring03/1630167385146.png)

#### 4.2.3 通知类型的使用

##### 前置通知

修改MyAdvic，在before方法上添加`@Before注解`
...
![ ](./assets/spring03/1630167805723.png)
...
![ ](./assets/spring03/1630167887131.png)
...
![ ](./assets/spring03/1630167969051.png)
...
![ ](./assets/spring03/1630168248052.png =600x)

再次运行，程序可以看到原始方法已经被执行了

![ ](./assets/spring03/1630168293492.png)
...
所以使用环绕通知要根据原始方法的返回值来设置环绕通知的返回值
...
> 为什么返回的是Object而不是int? 主要原因是Object类型更通用
...
![ ](./assets/spring03/1630169124446.png)
...
![ ](./assets/spring03/1630169357146.png)
...
因为环绕通知是可以控制原始方法执行的，把增强的代码写在调用原始方法的不同位置就可以实现不同的通知类型的功能，如:

:::

![ ](./assets/spring03/1630170090945.png)
...
![ ](./assets/spring03/1630214631112.png)
...
![ ](./assets/spring03/1630215355776.png)

**注意:**因为程序每次执行的时候是不一样的，所以运行多次最终的结果是不一样的。
...
![ ](./assets/spring03/1630215743444.png)
...
![ ](./assets/spring03/1630233154992.png)
...
![ ](./assets/spring03/1630233291929.png)
...
![1630233548743](./assets/spring03/1630233548743.png)
...
![ ](./assets/spring03/1630233974310.png)

**注意:**

* pjp.proceed()方法是有两个构造方法，分别是:

  ![ ](./assets/spring03/1630234756123.png)
...
![ ](./assets/spring03/1630237320870.png)
...
![ ](./assets/spring03/1630237586682.png)

运行App后查看运行结果，说明返回值已经被获取到

![ ](./assets/spring03/1630237372286.png)
...
![ ](./assets/spring03/1630239939043.png)

运行App后，查看控制台，就能看的异常信息被打印到控制台

![ ](./assets/spring03/1630239997560.png)
...
![ ](./assets/spring03/1630240203033.png)

问题描述:

* 点击链接，会提示，请输入提取码，如下图所示

  ![ ](./assets/spring03/1630240528228.png)
...
![ ](./assets/spring03/1630241681697.png)

现在项目的效果是，当输入密码为`root`控制台打印为true,如果密码改为`root`控制台打印的是false

我们想在的需求是使用AOP将参数进行统一处理，不管输入的密码`root`前后包含多少个空格，最终控制台打印的都是true。
...
![1630242491831](./assets/spring03/1630242491831.png)
...
* 转账业务，在转业务中会有两次数据层的调用，一次是加钱一次是减钱
...
![ ](./assets/spring03/1630243651541.png)

commit是用来提交事务，rollback是用来回滚事务。

PlatformTransactionManager只是一个接口，Spring还为其提供了一个具体的实现:

![ ](./assets/spring03/1630243993380.png)
...
![ ](./assets/spring03/1630247220645.png)
...
@Transactional可以写在接口上、接口方法上、类上和类方法上

* 写在接口上，该接口的所有实现类的所有方法都会有事务
...
**注意：**事务管理器要根据实现技术进行选择，Mybatis框架使用的是JDBC事务，可以直接使用`DataSourceTransactionManager`
...
1. 未开启Spring事务之前:
    ...
    ![ ](./assets/spring03/1630248794837.png)

    * AccountDao的outMoney因为是修改操作，会开启一个事务T1
    * AccountDao的inMoney因为是修改操作，会开启一个事务T2
    * AccountService的transfer没有事务，
    * 运行过程中如果没有抛出异常，则T1和T2都正常提交，数据正确
    * 运行过程中如果在两个方法中间抛出异常，T1因为执行成功提交事务，T2因为抛异常不会被执行
    * 就会导致数据出现错误

2. 开启Spring的事务管理后

    ![ ](./assets/spring03/1630249111055.png)
...
上一节我们介绍了两个概念，事务的管理员和事务的协同员，对于这两个概念具体做什么的，我们待会通过案例来使用下。处理这两个概念，还有就是事务的其他相关配置都有哪些，就是我们接下来要学习的内容。

在这一节中，我们主要学习三部分内容`事务配置`、`转账业务追加日志`、`事务传播行为`。

#### 6.3.1 事务配置

![ ](./assets/spring03/1630250069844.png)
...
![1630253779575](./assets/spring03/1630253779575.png)
...
![ ](./assets/spring03/1630254257628.png)
